...
const MenuIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d="M4 6h16M4 12h16M4 18h16" /></svg>
)
const CloseIcon = () => (
  <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18" /></svg>
)
...
function FomoBanner() {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => { const diff = getEndOfDay().getTime() - Date.now(); if (diff <= 0) return; setTimeLeft({ hours: Math.floor(diff / 3600000), minutes: Math.floor((diff % 3600000) / 60000), seconds: Math.floor((diff % 60000) / 1000) }) }
    updateTimer(); const iv = setInterval(updateTimer, 1000); const tv = setInterval(() => setTextIdx(i => (i + 1) % 3), 8000)
...
export default function LocationPage() {
  ...
  const galleryImages = [
    { src: '/images/gallery-1.jpg', alt: `Roller shutter ${cityName}` },
    { src: '/images/gallery-2.jpg', alt: `Roller shutter installation ${cityName}` },
    { src: '/images/gallery-3.jpg', alt: `Roller shutter repair ${cityName}` },
    { src: '/images/gallery-4.webp', alt: `Roller shutter project ${cityName}` },
    { src: '/images/gallery-5.jpg', alt: `Roller shutter service ${cityName}` },
    { src: '/images/gallery-6.jpg', alt: `Roller shutter maintenance ${cityName}` },
  ]
    ...
    <>
      ...
      <header className="sticky top-0 z-50" style={{ background: 'rgba(44,51,56,0.97)', backdropFilter: 'blur(12px)', boxShadow: 'var(--shadow-nav)' }}>
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between gap-4">
          <a href={`/${locale}`} className="flex items-center gap-2 shrink-0" aria-label={t('nav.brandName')}>
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--brand-yellow)' }}><ShutterIcon /></div>
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-[60]" style={{ background: 'rgba(28,31,34,0.95)' }}>
            <div className="flex justify-end p-4"><button onClick={() => setMobileMenuOpen(false)} className="icon-btn text-white p-2 cursor-pointer" aria-label="Close menu"><CloseIcon /></button></div>
      ...
      <main>
        ...
        <section className="relative overflow-hidden" style={{ background: 'var(--gradient-hero)', minHeight: '440px' }} aria-label="Hero">
          <div className="absolute inset-0 pointer-events-none corrugated-texture" aria-hidden="true" />
        ...
        <section aria-label="Statistics" style={{ background: 'var(--brand-gunmetal)', borderTop: '2px solid var(--brand-yellow)' }}>
          <div className="max-w-6xl mx-auto px-6 py-10">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
              {[0, 1, 2, 3].map(i => (
                <div key={i}><div className="stat-number text-3xl md:text-4xl mb-1 font-extrabold">{t(`stats.items.${i}.value`)}</div><div className="text-xs font-medium uppercase tracking-widest" style={{ color: 'var(--brand-steel-light)' }}>{t(`stats.items.${i}.label`)}</div></div>
              ))}
            </div>
          </div>
        </section>
        ...
        <section id="products" className="py-16 px-6" style={{ background: 'var(--brand-surface)' }} aria-labelledby="products-heading">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-sm font-normal max-w-2xl mx-auto" style={{ color: 'var(--brand-text-muted)' }}>{t('products.subheading')}</p>
            </div>
        ...
        <section className="py-16 px-6" style={{ background: '#fff' }}>
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: 'var(--brand-charcoal)', letterSpacing: '-0.025em' }}>{t('howItWorks.heading')}</h2>
              <p className="text-sm font-normal max-w-2xl mx-auto" style={{ color: 'var(--brand-text-muted)' }}>{t('howItWorks.subheading')}</p>
            </div>
            <div className="grid md:grid-cols-3 gap-8">
              {[0, 1, 2].map(i => (
                <div key={i} className="text-center relative">
                  ...
                  <h3 className="text-sm font-bold mb-2" style={{ color: 'var(--brand-charcoal)' }}>{t(`howItWorks.steps.${i}.title`)}</h3>
        ...
        <section className="py-16 px-6" style={{ background: 'var(--brand-surface-warm)', borderTop: '2px solid var(--brand-crimson)' }}>
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-10 items-center">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: 'var(--brand-charcoal)', letterSpacing: '-0.025em' }}>{t('riskProblem.heading')}</h2>
              <p className="text-sm font-normal mb-6" style={{ color: 'var(--brand-text-muted)' }}>{t('riskProblem.subheading')}</p>
        ...
        <section id="reviews" className="py-16 px-6" style={{ background: 'var(--brand-charcoal)' }}>
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-2" style={{ letterSpacing: '-0.025em' }}>{t('reviews.heading')}</h2>
            ...
            <article className="relative rounded-2xl p-8 md:p-10 overflow-hidden mb-6" style={{ background: 'rgba(242,199,68,0.08)', border: '1px solid rgba(242,199,68,0.2)' }}>
              ...
              <blockquote className="text-base md:text-lg font-medium text-white mb-6" style={{ lineHeight: '1.8', maxWidth: '720px' }}>&ldquo;{t('reviews.items.0.text')}&rdquo;</blockquote>
              <div className="flex items-center gap-3">
            ...
            {/* Remaining reviews — last spans full */}
        ...
        <section className="py-16 px-6" style={{ background: '#fff' }}>
          <div className="max-w-6xl mx-auto">
            <div className="mb-12">
              <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: 'var(--brand-charcoal)', letterSpacing: '-0.025em' }}>{t('whyChoose.heading')}</h2>
              <p className="text-sm font-normal max-w-2xl" style={{ color: 'var(--brand-text-muted)' }}>{t('whyChoose.subheading')}</p>
            </div>
            <div className="grid md:grid-cols-2 gap-x-10 gap-y-0">
              {[0, 1, 2, 3, 4, 5].map(i => (
                <div key={i} className="flex gap-5 py-6" style={{ borderBottom: '1px solid var(--brand-border)' }}>
        ...
        <section className="py-16 px-6" style={{ background: 'var(--brand-gunmetal)' }}>
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-3" style={{ letterSpacing: '-0.025em' }}>{t('gallery.heading')}</h2>
              <p className="text-sm font-normal" style={{ color: 'var(--brand-steel-light)' }}>{t('gallery.subheading')}</p>
            </div>
        ...
        <section id="locations" className="py-16 px-6" style={{ background: 'var(--brand-surface)' }}>
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: 'var(--brand-charcoal)', letterSpacing: '-0.025em' }}>{t('locations.heading')}</h2>
              <p className="text-sm font-normal max-w-2xl mx-auto" style={{ color: 'var(--brand-text-muted)' }}>{t('locations.subheading')}</p>
            </div>
          ...
          <section className="py-12 px-6" style={{ background: 'var(--brand-surface-warm)' }}>
            <div className="max-w-6xl mx-auto">
              <div className="text-center mb-8">
                <h2 className="text-xl md:text-2xl font-bold mb-2" style={{ color: 'var(--brand-charcoal)', letterSpacing: '-0.025em' }}>{t('nearbyLocations.heading')}</h2>
                <p className="text-sm font-normal" style={{ color: 'var(--brand-text-muted)' }}>{t('nearbyLocations.subheading')}</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                {nearby.map(loc => (
                  <a key={loc.slug} href={`/${locale}/roller-shutter/${loc.slug}`} className="product-card loc-link bg-white rounded-lg p-4 flex flex-col" style={{ border: '1px solid var(--brand-border)', borderLeft: '4px solid var(--brand-yellow)', transition: 'transform 200ms ease' }}>
                    <span className="text-sm font-bold mb-1" style={{ color: 'var(--brand-charcoal)' }}>{loc.name}</span>
                    <span className="text-[11px] font-normal" style={{ color: 'var(--brand-text-muted)' }}>Roller Shutter {loc.name}</span>
        ...
        <section id="faq" className="py-16 px-6" style={{ background: '#fff' }}>
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: 'var(--brand-charcoal)', letterSpacing: '-0.025em' }}>{t('faq.heading')}</h2>
            </div>
            <div>
              {[0, 1, 2, 3, 4, 5].map(i => (
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-1.5">{[0,1,2,3,4].map(i => <li key={i}><span className="text-xs font-normal" style={{ color: 'var(--brand-steel-light)' }}>{t(`footer.services.items.${i}`)}</span></li>)}</ul>
            </div>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-widest text-white mb-3">{t('footer.productTypes.heading')}</h4>
              <ul className="space-y-1.5">{[0,1,2,3,4,5].map(i => <li key={i}><span className="text-xs font-normal" style={{ color: 'var(--brand-steel-light)' }}>{t(`footer.productTypes.items.${i}`)}</span></li>)}</ul>
            </div>
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-widest text-white mb-3">{t('footer.areas.heading')}</h4>
              <ul className="space-y-1.5">{[0,1,2,3,4,5].map(i => <li key={i}><span className="text-xs font-normal" style={{ color: 'var(--brand-steel-light)' }}>{t(`footer.areas.items.${i}`)}</span></li>)}</ul>
            </div>
          </div>
